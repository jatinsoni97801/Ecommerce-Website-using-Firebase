  ...
  return (
    ...
    <div className='product-text quantity-box'>
        <div className='action-btns minus' onClick={handleCartProductDecrease} >
           <button><Icon icon={minus} size={20}/></button> 
        </div>                
        <div>{cartProduct.qty}</div>               
        <div className='action-btns plus' onClick={handleCartProductIncrease} >
          <button><Icon icon={plus} size={20}/></button>  
        </div>
    </div>
    <div className='product-text cart-price'>$ {cartProduct.TotalProductPrice}</div>
    < div className='btn btn-danger btn-md cart-btn' onClick={handleCartProductDelete}><button><img src={del} alt="" /></button></div>             
</div>
  )
}
